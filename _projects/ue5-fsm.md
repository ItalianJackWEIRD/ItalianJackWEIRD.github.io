...
States and the initial state are configured entirely from the Details Panel — no hardcoded references in C++. `InitStateManager()` is `BlueprintCallable` and called from the owning Actor's `BeginPlay`.
