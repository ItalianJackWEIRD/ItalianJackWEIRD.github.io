...
<div class="gallery gallery--single">
  <img src="/assets/img/orientation-direction.png" alt="On-screen debug of OrientationDirection">
</div>
...
The braking parameters feeding the prediction are read **live** through Unreal's Property Access system, from a pointer to the Character Movement Component cached once in `NativeInitializeAnimation`. They're deliberately not copied into the Anim Instance: each state swaps the movement parameters on enter (via a data asset), and a cached copy would go stale the moment that happened.

<div class="gallery">
  <img src="/assets/img/distance-curve.png" alt="Distance curve generated by the modifier">
  <img src="/assets/img/set-stop-distance.png" alt="Set Stop Distance anim function">
</div>
<p class="gallery-caption">Left: the auto-generated Distance curve. Right: the anim function predicting the stop location and feeding the match node.</p>
...
**Orientation Warping** rotates the lower body to line the legs up with the actual direction of travel while the upper body stays oriented to where the character faces. It runs per direction across the movement states, fed by re-centred angles derived from the same signed angle the orientation system already computes.
