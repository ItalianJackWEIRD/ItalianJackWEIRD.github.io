...
The goal was simple to state and genuinely hard to implement: take a static 3D spider model and make it walk convincingly across any terrain, without a single keyframe animation. Everything from foot placement, body alignment, leg timing and step arcs had to be computed at runtime.
...
At this point all 8 legs moved simultaneously, which looked completely robotic. The fix was a **FootTimings** array in which each foot has its own timer that increments every frame by `deltaTime`. When a timer exceeds a threshold, that foot recalculates its target position and resets.
